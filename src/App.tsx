...
function App() {
  ...
  async function getMeals() {
    ...
    try {
      ...
      const response = await fetch('/api/recommend-meals', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ ingredients, remaining: targetSet ? remaining : null, mealRequest }),
      })
      const data = await response.json().catch(() => ({}))
  ...
  return <main>
    ...
    <section className="hero" id="top"><div><p className="eyebrow">YOUR KITCHEN, SIMPLIFIED</p><h1>What can we make<br /><em>today?</em></h1><p className="intro">Scan your fridge, confirm what you have, and get meal ideas that work with your day.</p></div><div className="hero-art" aria-hidden="true"><div className="sun" /><div className="fridge"><span>FRIDGE</span><i /><b /><div className="orioles-sticker" aria-label="Baltimore Orioles inspired O’s sticker">O’s</div><img className="ravens-sticker" src={ravensLogo} alt="Baltimore Ravens logo" /></div><img className="dog-mascot" src={dogMascot} alt="" /><span className="tomato">●</span><span className="leaf">✦</span></div></section>
