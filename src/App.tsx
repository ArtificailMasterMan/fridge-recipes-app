...
function App() {
  ...
  function selectPhoto(file?: File) {
    setScanError(''); setScanState('idle')
    if (!file) return
    if (file.size > 8 * 1024 * 1024) { setScanFile(null); setScanState('error'); setScanError('Choose an image smaller than 8 MB.'); return }
    if (['image/heic', 'image/heif'].includes(file.type) || /\.hei[cf]$/i.test(file.name)) { setScanFile(null); setScanState('error'); setScanError('HEIC photos are not supported yet. Choose a JPEG/PNG or upload an iPhone screenshot.'); return }
    if (!['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(file.type)) { setScanFile(null); setScanState('error'); setScanError('Choose a JPG, PNG, WEBP, or GIF image.'); return }
    setScanFile(file)
    setPhoto((current) => { if (current) URL.revokeObjectURL(current); return URL.createObjectURL(file) })
  }
